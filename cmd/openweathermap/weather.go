...
func (env *environment) collectWeather(s station) collectorFunc {
	...
	metricWeatherDescription := env.Metrics.WeatherDescription.MustCurryWith(labels)
	...
	return func(ctx context.Context) error {
		...
		env.Metrics.Sunrise.With(labels).Set(data.Current.Sunrise)
		env.Metrics.Sunset.With(labels).Set(data.Current.Sunset)
		...
		env.Metrics.WindDirection.With(labels).Set(data.Current.WindDirection)

		metricWeather.Reset()
		metricWeatherIcon.Reset()
		metricWeatherDescription.Reset()
		for _, w := range data.Current.Weather {
			...
			metricWeatherDescription.With(prometheus.Labels{
				"description": fmt.Sprint(w.Description),
			}).Set(1)
